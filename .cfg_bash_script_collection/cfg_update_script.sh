...
# 'script always run locally, get version from package.json
...
if [[ -f "$HOME/package.json" ]]; then
  ...
  CFG_VERSION_MESSAGE="-- v${CFG_SCRIPT_VERSION} -- local"
else
...
output_help() {
  printf "\n Usage :  bash .cfg_bash_script_collection/cfg_update_script.sh [options]\n"
